...
export async function generateMetadata({ params }) {
  ...
  const title = "Blogs | Pixel Pulse Play";
  const description =
...
const schema = {
  ...
  url: `${(process.env.SITE_URL || "https://www.pixelpulseplay.ca").replace(/\/$/, "")}/blogs`,
  blogPost: blogsToRender?.map((blog) => {
    return {
      "@type": "BlogPosting",
      headline: blog.title,
      url: `${(process.env.SITE_URL || "https://www.pixelpulseplay.ca").replace(/\/$/, "")}${getBlogHref(blog)}`,
      image:
        blog.featuredImage ||
        "https://storage.googleapis.com/pixel-pulse-play/web/h-Logo.png",
